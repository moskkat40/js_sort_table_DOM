...
head.addEventListener("click", (e)=>{
    ...
    rows.sort((a, b)=>{
        ...
        if (index === 0 || index === 1) return aText.localeCompare(bText);
        else if (index === 2 || index === 3) return convertToNumber(aText) - convertToNumber(bText);
    });
    holeList.append(...rows);
});
function convertToNumber(salaryString) {
    if (salaryString.startsWith("$")) return parseInt(salaryString.replace(/[$,]/g, ""));
